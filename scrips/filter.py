...
nome_do_arquivo_entrada = '../datasets/votos_dep_fed_CEBRAP.csv'
nome_do_arquivo_saida = 'resultado_filtrado.xlsx'
caminho_de_saida = f'../result/{nome_do_arquivo_saida}'
...
try:
    ...
    print(f"Filtro aplicado! Foram encontrados {len(df_filtrado)} registros.")

    if not df_filtrado.empty:
        print(f"\n--- Exportando o resultado para '{caminho_de_saida}'...")

        df_filtrado.to_excel(caminho_de_saida, index=False)

        print(f"SUCESSO! O arquivo '{nome_do_arquivo_saida}' foi criado na pasta 'result'.")
    else:
